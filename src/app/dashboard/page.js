...
function DashboardContent() {
  ...
  const [countdowns, setCountdowns] = useState({
    ...
    gfg: 86400 * 3 + 36000 // 3 days 10 hours
  });
  ...
  useEffect(() => {
    const timer = setInterval(() => {
      setCountdowns((prev) => ({
        ...
        gfg: prev.gfg > 0 ? prev.gfg - 1 : 120000
      }));
    }, 1000);
  ...
  const toggleReminder = (platform) => {
    setReminders((prev) => {
      ...
      // Trigger a simulated notification banner / log
      return { ...prev, [platform]: newVal };
    });
  };

  const potd = [
    { title: "Sliding Window Maximum", platform: "LeetCode", difficulty: "Hard", link: "https://leetcode.com/problems/sliding-window-maximum/", tags: ["Queue", "Sliding Window"] },
    { title: "Minimum Operations to Halve Array Sum", platform: "GeeksforGeeks", difficulty: "Medium", link: "https://www.geeksforgeeks.org/problems/minimum-operations-to-halve-array-sum/1", tags: ["Heap", "Greedy"] }
  ];
  ...
  const topicHealth = [
    { name: "Arrays & Hashing", solved: 184, total: 200, status: "Master", color: "var(--success)" },
    { name: "Trees & Graphs", solved: 142, total: 180, status: "Strong", color: "var(--info)" },
    { name: "Dynamic Programming", solved: 48, total: 150, status: "Weak", color: "var(--danger)" },
    { name: "Math & Bit Manipulation", solved: 84, total: 120, status: "Average", color: "var(--warning)" }
  ];
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
      ...
      <div className="glass-card" style={{ display: "flex", flexWrap: "wrap", justifyContent: "space-between", alignItems: "center", gap: "1.25rem", background: "linear-gradient(135deg, rgba(99, 102, 241, 0.1) 0%, rgba(139, 92, 246, 0.05) 100%)", borderColor: "rgba(99, 102, 241, 0.2)" }}>
        <div style={{ flexGrow: "1" }}>
          <h2 style={{ fontSize: "1.6rem", marginBottom: "0.25rem", textAlign: "left" }}>
            Welcome back, Aishvary! 👋
          </h2>
          <p style={{ color: "var(--text-secondary)", fontSize: "0.9rem", margin: "0" }}>
            You've solved <span style={{ color: "white", fontWeight: "600" }}>342 problems</span> this week. You are outperforming 94% of your peers! 🚀
          </p>
        </div>
        ...
        <div style={{ width: "220px" }}>
          <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.75rem", marginBottom: "4px" }}>
            ...
            <span style={{ color: "white", fontWeight: "600" }}>85% (342/400)</span>
          </div>
      ...
      {/* 3. Problem of the Day + Upcoming Contests Strip */}
      <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "1.5rem" }} className="grid-responsive-two">
        ...
        <div className="glass-card" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }} onClick={() => router.push("/potd")} className="link">View History</span>
          </div>

          <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem", flexGrow: "1" }}>
            {potd.map((prob) => (
              <div key={prob.title} style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.85rem", background: "rgba(8, 11, 17, 0.4)", borderRadius: "10px", border: "1px solid var(--border-ice)" }}>
                <div>
                  <div style={{ display: "flex", alignItems: "center", gap: "0.5rem", marginBottom: "4px" }}>
                    <span className={`badge ${prob.platform === "LeetCode" ? "badge-leetcode" : "badge-gfg"}`} style={{ fontSize: "0.65rem" }}>
                      {prob.platform}
                    </span>
                    <span className={`badge ${prob.difficulty === "Hard" ? "badge-hard" : "badge-medium"}`} style={{ fontSize: "0.65rem" }}>
                      {prob.difficulty}
                    </span>
                  </div>
                  <span style={{ fontSize: "0.85rem", fontWeight: "600" }}>{prob.title}</span>
                </div>
                <a href={prob.link} target="_blank" rel="noreferrer" className="btn btn-secondary btn-sm" style={{ padding: "0.35rem 0.6rem" }}>
        ...
        <div className="glass-card" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }} onClick={() => router.push("/contests")} className="link">Full Calendar</span>
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: "0.75rem" }}>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.75rem", borderRadius: "10px", background: "rgba(49, 141, 236, 0.04)", border: "1px solid rgba(49, 141, 236, 0.1)" }}>
              <div>
                <span className="badge badge-codeforces" style={{ fontSize: "0.6rem", padding: "0.15rem 0.4rem", marginBottom: "3px" }}>Codeforces</span>
                <div style={{ fontSize: "0.8rem", fontWeight: "600" }}>CF Round 998 (Div 2)</div>
            ...
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", padding: "0.75rem", borderRadius: "10px", background: "rgba(245, 158, 11, 0.04)", border: "1px solid rgba(245, 158, 11, 0.1)" }}>
              <div>
                <span className="badge badge-leetcode" style={{ fontSize: "0.6rem", padding: "0.15rem 0.4rem", marginBottom: "3px" }}>LeetCode</span>
                <div style={{ fontSize: "0.8rem", fontWeight: "600" }}>Weekly Contest 432</div>
      ...
      {/* 4. Lower Grid: Submissions Feed + Topic Health */}
      <div style={{ display: "grid", gridTemplateColumns: "1.2fr 0.8fr", gap: "1.5rem" }} className="grid-responsive-two">
        ...
        <div className="glass-card" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
          <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
            ...
            <span style={{ fontSize: "0.75rem", color: "var(--text-muted)" }} onClick={() => router.push("/profile")} className="link">View Analytics</span>
          </div>
        ...
        {/* Right Pane: Topic Health & AI recommendation */}
        <div style={{ display: "flex", flexDirection: "column", gap: "1.5rem" }}>
          
          {/* Topic Health */}
          <div className="glass-card" style={{ display: "flex", flexDirection: "column", gap: "1rem" }}>
            <h3 style={{ fontSize: "1.1rem" }}>Topic Health Diagnostic</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: "0.85rem" }}>
              {topicHealth.map((topic) => (
                <div key={topic.name}>
                  <div style={{ display: "flex", justifyContent: "space-between", fontSize: "0.75rem", marginBottom: "4px" }}>
                    <span style={{ fontWeight: "500" }}>{topic.name}</span>
                    <span style={{ color: topic.color, fontWeight: "600" }}>{topic.status} ({topic.solved}/{topic.total})</span>
                  </div>
          ...
          <div className="glass-card" style={{
            ...
            <p style={{ fontSize: "0.8rem", color: "var(--text-secondary)", margin: "0", lineHeight: "1.4" }}>
              "Based on your high TLE rates on Recursion problems and low rating on Dynamic Programming tags, we recommend practicing <span style={{ color: "white", fontWeight: "600" }}>Sliding Window problems</span>. It will build key state memoization fundamentals."
            </p>

            <button
              className="btn btn-primary btn-sm"
              style={{ alignSelf: "flex-start", marginTop: "0.5rem", padding: "0.4rem 1rem", fontSize: "0.75rem" }}
              onClick={() => router.push("/recommendations")}
            >
              <Sparkles size={12} />
              <span>Launch Study Path</span>
            </button>
          </div>
      ...
      {/* Embedded CSS for responsiveness handles on the page grid */}
      <style jsx global>{`
