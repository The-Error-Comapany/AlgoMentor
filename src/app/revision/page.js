...
function RevisionHubContent() {
  ...
  const handleAddExternalProblem = async (e) => {
    ...
    try {
      const tagsArray = extTags
        ? extTags.split(",").map(t => t.trim().toLowerCase()).filter(t => t.length > 0)
        : ["external"];
            ...
            <form onSubmit={handleAddExternalProblem}>
              <div className="rev-modal-body">
                ...
                <div className="rev-form-row">
                  ...
                  <div className="rev-form-group">
                    <label className="rev-form-label">Tags (comma separated)</label>
                    <input
                      type="text"
                      placeholder="e.g. Graph, BFS"
                      value={extTags}
